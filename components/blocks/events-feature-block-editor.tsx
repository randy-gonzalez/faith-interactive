...
/**
 * Events Feature Block Editor Component
 *
 * Edit form for events feature block that displays upcoming events.
 */

import { useState } from "react";
import type { Block, EventsFeatureBlock, BlockBackground, BlockAdvanced } from "@/types/blocks";
...
interface EventsFeatureBlockEditorProps {
  block: Block;
...
export function EventsFeatureBlockEditor({
  block,
  onChange,
  disabled,
}: EventsFeatureBlockEditorProps) {
  const eventsFeatureBlock = block as EventsFeatureBlock;
  const { data } = eventsFeatureBlock;
  const [activeTab, setActiveTab] = useState<"content" | "background" | "advanced">("content");

  function updateData(updates: Partial<EventsFeatureBlock["data"]>) {
    onChange({
      ...eventsFeatureBlock,
      data: { ...data, ...updates },
    });
    ...
    <div className="space-y-4">
      ...
      {activeTab === "background" && (
        <BlockBackgroundEditor
          background={eventsFeatureBlock.background || createDefaultBackground()}
      ...
      {activeTab === "advanced" && (
        <BlockAdvancedEditor
          advanced={eventsFeatureBlock.advanced || {}}
