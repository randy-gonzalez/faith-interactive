/**
 * Platform Media Picker Component
 *
 * A modal-based component for selecting images from the platform media library.
 * Used for marketing site assets (case studies, blog posts, etc.)
 * Uses /api/platform/media instead of tenant-scoped /api/media
 */
...
interface PlatformMediaPickerProps {
  value: string | null;
...
export function PlatformMediaPicker({
  ...
  async function fetchMedia() {
    setLoading(true);
    try {
      const response = await fetch("/api/platform/media?type=image");
      if (response.ok) {
        const data = await response.json();
        setMedia(data.data?.media || []);
      }
    } catch (error) {
      console.error("Failed to fetch platform media:", error);
    } finally {
      setLoading(false);
    }
  }
...
/**
 * Multi-Image Picker for galleries
 */
interface PlatformMultiMediaPickerProps {
  value: string[];
...
export function PlatformMultiMediaPicker({
  value,
  onChange,
  label,
  disabled = false,
  maxImages = 20,
}: PlatformMultiMediaPickerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [media, setMedia] = useState<MediaItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  ...
  // Sync selected items when value changes
  useEffect(() => {
  ...
  async function fetchMedia() {
    setLoading(true);
    try {
      const response = await fetch("/api/platform/media?type=image&limit=100");
      if (response.ok) {
        const data = await response.json();
        setMedia(data.data?.media || []);
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data?.media) {
          ...
          setSelectedItems((prev) => new Set([...prev, newItem.url]));
        }
      } else {
        const data = await response.json();
        setUploadError(data.error || "Upload failed");
  ...
  function handleClose() {
    setIsOpen(false);
    setSelectedItems(new Set(value)); // Reset to original value
    setUploadError(null);
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <Button
        ...
        disabled={disabled}
      >
        {value.length > 0 ? `Manage Images (${value.length})` : "Add Images"}
      </Button>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div className="relative min-h-screen flex items-center justify-center p-4">
            <div className="relative bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[80vh] flex flex-col">
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900">
                  Select Images ({selectedItems.size}/{maxImages})
                </h2>
                <button
                  onClick={handleClose}
                  className="text-gray-400 hover:text-gray-600"
